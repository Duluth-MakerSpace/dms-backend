...
let PostResolver = class PostResolver {
    ...
    post(uuid) {
        return Post_1.Post.findOne({ where: { uuid: uuid } });
    }
    async createPost(title) {
        return Post_1.Post.create({ title: title }).save();
    }
    async updatePost(uuid, title) {
        const post = await Post_1.Post.findOne({ where: { uuid: uuid } });
        if (!post) {
            return null;
        }
        if (typeof title !== 'undefined') {
            await Post_1.Post.update({ uuid }, { title });
        }
        return post;
    }
